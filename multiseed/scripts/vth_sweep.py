import sys
#!/usr/bin/env python3
"""AUTO-GENERATED from Vth_sweep_GHZ.ipynb.
...
# === cell #15 ===
# =====================================================================
# V_TH (FIRING THRESHOLD) SWEEP CONFIGURATION
# =====================================================================

V_TH_LIST = [0.3, 0.5, 0.7, 1.0, 1.5]
N_LIST = [3, 5, 8]
METHODS = ['M1', 'M2']
M_M1 = 12
M_M2 = 2
...
# Fixed SNN hyperparams (same for all V_th)
T_FIXED = 16                 # chosen from T-sweep optimum
ENC_GAMMA = 1.5
TAU_MEM_INV = 100.0
TAU_SYN_INV = 200.0
...
total = len(V_TH_LIST) * len(N_LIST) * len(METHODS) * len(MODEL_CONFIGS)
print(f"V_th-Sweep Configuration:")
print(f"  V_th values: {V_TH_LIST}")
...
# CHECKPOINT_DIR = Path("checkpoints_Vth_sweep")
...
def make_model_fn(config, vth_val, N):
    """Build a model factory. V_th is swept; all other hyperparams fixed.
    NOTE: Option A — the crossbar's normally-adapted v_th is overridden by vth_val."""
    name = config['name']
    bits = config['bits']
    ...
    if name == 'Norse-Simple2D':
        def fn(cond_dim, d):
            return SCNNGen_LIF_Simple2D(
                ...
                T=T_FIXED, v_th=vth_val, enc_gamma=ENC_GAMMA,
                tau_mem_inv=TAU_MEM_INV, tau_syn_inv=TAU_SYN_INV)
        return fn

    # Crossbar: keep the per-bits enc_gamma tuning (it is independent of V_th),
    # but use the swept V_th directly so the sweep axis is clean.
    gamma = ENC_GAMMA * 2 if bits <= 4 else ENC_GAMMA * 1.5

    def fn(cond_dim, d):
        return SCNNGen_Crossbar_Simple2D(
            cond_dim=cond_dim, d=d,
            proj_hw=(N*2, N*2), ch=(32, 64),
            T=T_FIXED, v_th=vth_val, enc_gamma=gamma,
            tau_mem_inv=TAU_MEM_INV, tau_syn_inv=TAU_SYN_INV,
            weight_bits=bits, adc_bits=bits, dac_bits=bits,
...
def run_Vth_sweep(vth_list=None, N_list=None, methods=None,
                  model_configs=None, use_amp=True, checkpoint_every=10):
    vth_list = vth_list or V_TH_LIST
    ...
    total = len(vth_list) * len(N_list) * len(methods) * len(model_configs)
    run_idx = 0
    ...
    for method in methods:
        ...
        for N in N_list:
            ...
            for vth_val in vth_list:
                for config in model_configs:
                    run_idx += 1
                    name = config['name']
                    if (SEED, name, N, method, float(vth_val)) in _done_keys:
                        print(f"  [skip] seed={SEED} {name} N={N} {method} V_th={vth_val}", flush=True)
                        continue
                    print(f"\n[{run_idx}/{total}] {method} | N={N} | V_th={vth_val} | {name}")

                    model_fn = make_model_fn(config, vth_val, N)
                    ...
                    try:
                        ...
                        ckpt_name = f'seed{SEED}_{name}_N{N}_{method}_Vth{vth_val}.pt'
                        torch.save({
                            'state_dict': model.state_dict(),
                            'model_name': name, 'N': N, 'method': method,
                            'T': T_FIXED, 'V_th': vth_val,
...
# df_sweep = pd.read_csv(CHECKPOINT_DIR / 'Vth_sweep_final.csv')
# display(df_sweep)
...
for method in METHODS:
    ...
    pivot = df_m.pivot_table(
        index=['Model', 'N'], columns='V_th',
        values='F_best', aggfunc='first')

print("\n" + "=" * 80)
print("INFERENCE ENERGY (uJ) vs V_th")
print("=" * 80)
for method in METHODS:
    print(f"\n--- Method: {method} ---")
    df_m = df_sweep[df_sweep['method'] == method]
...
for method in METHODS:
    ...
    for ax_idx, N in enumerate(N_LIST):
        ...
        for cfg in MODEL_CONFIGS:
            name = cfg['name']
            df_model = df_n[df_n['Model'] == name].sort_values('V_th')
            if len(df_model) == 0: continue
            infidelity = np.clip(1.0 - df_model['F_best'].values, 1e-6, None)
            ls = '-' if method == 'M1' else '--'
            ax.plot([V_TH_LIST.index(v) for v in df_model['V_th'].values], infidelity,
                    marker=MARKERS[name], color=COLORS[name],
                    linestyle=ls, label=f"{name} ({method})",
                    linewidth=1.5, markersize=6)
        ax.tick_params(axis='x', rotation=0)
        ax.set_yscale('log')
        ax.set_xlabel('Firing threshold V_th')
        ax.set_title(f'N = {N} qubits')
        ax.grid(True, alpha=0.3, which='both')
        ax.set_xticks(range(len(V_TH_LIST)))
        ax.set_xticklabels([str(v) for v in V_TH_LIST])

axes[0].set_ylabel('Infidelity (1 - F)')
handles, labels = axes[-1].get_legend_handles_labels()
fig.legend(handles, labels, loc='upper center',
           bbox_to_anchor=(0.5, -0.02), ncol=3, fontsize=9)
fig.suptitle('Infidelity vs Firing threshold V_th', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot1_Vth_infidelity.pdf', bbox_inches='tight', dpi=150)


# === cell #21 ===
# =====================================================================
# PLOT 4: Fidelity vs Energy Pareto (colored by V_th)
# =====================================================================
...
sm.set_array([])
cbar = fig.colorbar(sm, ax=axes, label='Firing threshold V_th', shrink=0.8)
...
fig.suptitle('Fidelity vs Inference Energy (colored by Firing threshold V_th)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot4_Vth_pareto.pdf', bbox_inches='tight', dpi=150)
...
print("=" * 80)
print("KEY FINDINGS: best V_th per (model, N)")
print("=" * 80)
...
for method in METHODS:
    ...
    for cfg in MODEL_CONFIGS:
        ...
        for N in N_LIST:
            ...
            print(f"  {name}, N={N}: best V_th={best_row['V_th']} "
...
def recompute_Vth_sweep_energy():
    """Reload models from checkpoints and recompute energy for every sweep point."""
    print("="*80)
    print("ENERGY RECOMPUTE FROM VTH_SWEEP CHECKPOINTS")
    print("="*80)
    rows = []
    for ckpt_path in sorted(CHECKPOINT_DIR.glob('*.pt')):
        ...
        model_fn = make_model_fn(cfg, ckpt.get('Vth', None), N)
    ...
    df = pd.DataFrame(rows)
    df.to_csv(CSV_PATH, index=False)
    print(f"\nRecomputed {len(df)} rows -> Vth_sweep_recomputed.csv")
    return df

# Usage: df = recompute_Vth_sweep_energy()
