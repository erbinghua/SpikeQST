import sys
#!/usr/bin/env python3
"""AUTO-GENERATED from p_sweep_SCNN_Mixed.ipynb.
...
- Target: Mixed GHZ states (Werner state, sweeping p, Eq. A4 Hua et al.)
...
def optimize_single_state_crossbar(model_class, N=3, method='M1', M=128, steps=500, lr=1e-3,
                                   ...
                                   use_amp=True, log_energy=True, p=0.5):
    """
    ...
    print(f"    Target: Mixed GHZ state (Werner p={p})")
...
def run_crossbar_benchmark(N_list=[3,4,5], methods=['M1','M2'],
    ...
    for method in methods:
        print(f"\n{'='*70}")
        print(f"CROSSBAR BENCHMARK (Mixed GHZ p={p}): {method}")
...
def compare_crossbar_vs_standard(N=3, method='M1', M=128, steps=500,
    ...
    """
    print(f"\n{'='*70}")
    print(f"QUANTIZATION IMPACT ANALYSIS (Mixed GHZ p={p}): N={N}, {method}")
    print(f"{'='*70}\n")
...
def optimize_single_state(model_class, N=3, method='M1', M=128, steps=500, lr=1e-3,
                          noise_std=0.0, normalize_cond=True, warm_spiking=False,
                          use_amp=True, use_compile=False, p=0.5):
    """
    Ottimizzazione con Mixed Precision Training e opzionalmente torch.compile.
    Target: Mixed GHZ state (Werner state, p=p pure-state weight).
...
# === cell #16 ===
# =============================================================================
# UNIFIED WRAPPER (with p threaded through)
# =============================================================================

def optimize_with_unified_output(model_class, N, method, M, steps, lr, p=0.5,
    ...
    if is_crossbar:
        res = optimize_single_state_crossbar(
            model_class=model_class, N=N, method=method, M=M,
            steps=steps, lr=lr, warm_spiking=warm_spiking,
            use_amp=use_amp, log_energy=False, p=p, **crossbar_kwargs)
        elapsed = time.time() - t0
        return {'model': res['model'], 'F_best': res['F_best'],
                'F_hist': res['F_hist'], 'loss_hist': res.get('loss_hist', []),
                'time_sec': elapsed, 'energy_final': res['energy_final']}
    else:
        res = optimize_single_state(
            model_class=model_class, N=N, method=method, M=M,
            steps=steps, lr=lr, warm_spiking=warm_spiking,
        ...
        return {'model': res['model'], 'F_best': F_best, 'F_hist': F_hist,
                'loss_hist': l_hist, 'time_sec': elapsed}
...
def werner_purity(p, d):
    ...
    Closed form: p^2 + (1-p)^2/d + 2 p (1-p)/d.
    """
    return p**2 + (1.0 - p)**2 / d + 2.0 * p * (1.0 - p) / d


# =============================================================================
# p-SWEEP RUNNER (fixed N=5, methods=[M1,M2], same models as benchmark)
# =============================================================================

def run_p_sweep_scnn(p_list, N=5, methods=['M1', 'M2'],
                    M_M1=256, M_M2=4, steps=500, lr=1e-3,
                    crossbar_bits=[8, 4], T=8, sparsity=0.1,
                    use_amp=True):
    """p-sweep matching the SCNN benchmark methodology exactly."""
    # === MULTISEED PATCHED ===
    if _ARGS.quick:
        p_list = [0.5]
        methods = ['M1']
    ...
    if CSV_PATH.exists():
        ...
        _done_outer = {k for k, c in _counts.items() if c >= 6}
    ...
    # Same fixed hyperparams as run_benchmark_with_energy_breakdown
    current_T = T
    current_vth = 0.9
    current_gamma = 1.5
    up2d_T = current_T
    ...
    run_idx = 0
    print(f"\n{'='*80}\np-SWEEP: SCNN benchmark, N={N}, T={T}, p_list={p_list}\n{'='*80}")
    ...
    for method in methods:
        ...
        for p in p_list:
            ...
            # ---- 1. CNN-Simple2D ----
            print("  [1/6] CNN-Simple2D")
            model_fn = lambda cd, d_: CNNGen_Simple2D(
                cond_dim=cd, d=d_, proj_hw=(N*2, N*2), ch=(32,64))
            res = optimize_with_unified_output(
                model_class=model_fn, N=N, method=method, M=M, p=p,
                steps=steps, lr=lr, warm_spiking=False,
                use_amp=use_amp, is_crossbar=False)
            E_inf = estimate_gpu_inference_energy(res['model'])
            rows.append({'seed': SEED, 'Model': 'CNN-Simple2D', 'Architecture': 'SCNN', 'HW': 'GPU',
                         'N': N, 'method': method, 'p': p, 'purity': purity,
                         'F_best': res['F_best'], 'F_last': res['F_hist'][-1] if res['F_hist'] else res['F_best'],
                         'E_inference_uJ': E_inf['E_total_uJ'],
                         'n_params': E_inf['n_params'], 'time_sec': res['time_sec']})
            print(f"      F={res['F_best']:.4f}, E_inf={E_inf['E_total_uJ']:.2f} uJ")
            del res; gpu_cleanup()

            # ---- 2. CNN-Up2D-Paper ----
            print("  [2/6] CNN-Up2D-Paper")
            model_fn = lambda cd, d_: CNNGen_Up2D_Paper(cond_dim=cd, d=d_)
            res = optimize_with_unified_output(
                model_class=model_fn, N=N, method=method, M=M, p=p,
                steps=steps, lr=lr, warm_spiking=False,
                use_amp=use_amp, is_crossbar=False)
            E_inf = estimate_gpu_inference_energy(res['model'])
            rows.append({'seed': SEED, 'Model': 'CNN-Up2D-Paper', 'Architecture': 'SCNN', 'HW': 'GPU',
                         'N': N, 'method': method, 'p': p, 'purity': purity,
                         'F_best': res['F_best'], 'F_last': res['F_hist'][-1] if res['F_hist'] else res['F_best'],
                         'E_inference_uJ': E_inf['E_total_uJ'],
                         'n_params': E_inf['n_params'], 'time_sec': res['time_sec']})
            ...
            # ---- 3. SCNN-Norse-Simple2D ----
            print("  [3/6] SCNN-Norse-Simple2D")
            model_fn = lambda cd, d_: SCNNGen_LIF_Simple2D(
                cond_dim=cd, d=d_, proj_hw=(N*2, N*2), ch=(32,64),
            ...
            rows.append({'seed': SEED, 'Model': 'SCNN-Norse-Simple2D', 'Architecture': 'SCNN', 'HW': 'Loihi',
                         'N': N, 'method': method, 'p': p, 'purity': purity,
                         'F_best': res['F_best'], 'F_last': res['F_hist'][-1] if res['F_hist'] else res['F_best'],
                         'E_inference_uJ': E_inf['E_total_uJ'],
                         'n_params': E_inf['n_params'], 'time_sec': res['time_sec']})
            print(f"      F={res['F_best']:.4f}, E_inf={E_inf['E_total_uJ']:.4f} uJ")
            del res; gpu_cleanup()

            # ---- 4. SCNN-Norse-Up2D ----
            print("  [4/6] SCNN-Norse-Up2D")
            _T, _vth, _g, _tm, _ts = up2d_T, up2d_vth, up2d_gamma, up2d_tau_mem, up2d_tau_syn
            model_fn = lambda cd, d_: SCNNGen_LIF_Up2D_Paper(
                cond_dim=cd, d=d_, T=_T, v_th=_vth,
                tau_mem_inv=_tm, tau_syn_inv=_ts, enc_gamma=_g)
            res = optimize_with_unified_output(
                model_class=model_fn, N=N, method=method, M=M, p=p,
                steps=steps, lr=lr, warm_spiking=True,
                use_amp=use_amp, is_crossbar=False)
            E_inf = estimate_loihi_inference_energy(res['model'], T=up2d_T, sparsity=sparsity)
            rows.append({'seed': SEED, 'Model': 'SCNN-Norse-Up2D', 'Architecture': 'SCNN', 'HW': 'Loihi',
                         'N': N, 'method': method, 'p': p, 'purity': purity,
                         'F_best': res['F_best'], 'F_last': res['F_hist'][-1] if res['F_hist'] else res['F_best'],
                         'E_inference_uJ': E_inf['E_total_uJ'],
                         'n_params': E_inf['n_params'], 'time_sec': res['time_sec']})
            ...
            # ---- 5,6. SCNN-Crossbar-{8,4}b ----
            for ci, bits in enumerate(crossbar_bits, start=5):
                print(f"  [{ci}/6] SCNN-Crossbar-Simple2D-{bits}b")
                if bits <= 4:
                    actual_vth, actual_gamma = 0.3, 3.0
                else:
                    actual_vth, actual_gamma = 0.5, 2.25
                _bits, _T2, _vth2, _g2 = bits, current_T, actual_vth, actual_gamma
                model_fn = lambda cd, d_: SCNNGen_Crossbar_Simple2D(
                    cond_dim=cd, d=d_, proj_hw=(N*2, N*2), ch=(32,64),
                    T=_T2, v_th=_vth2, enc_gamma=_g2,
                    weight_bits=_bits, adc_bits=_bits, dac_bits=_bits)
                _variation = cb8_variation if bits == 8 else 0.02
                _noise = cb8_noise if bits == 8 else 0.01
                res = optimize_single_state_crossbar(
                    model_class=model_fn, N=N, method=method, M=M, p=p,
                    steps=steps, lr=lr, warm_spiking=True,
                    weight_bits=bits, adc_bits=bits, dac_bits=bits,
                ...
                E_inf = estimate_crossbar_inference_energy(res['model'], T=current_T, sparsity=sparsity, bits=bits)
                F_hist = res['F_hist']
                F_last = float(F_hist[-1]) if F_hist else res['F_best']
                rows.append({'seed': SEED, 'Model': f'SCNN-Crossbar-Simple2D-{bits}b', 'Architecture': 'SCNN',
                             'HW': f'Crossbar-{bits}b', 'N': N, 'method': method, 'p': p, 'purity': purity,
                             'F_best': res['F_best'], 'F_last': F_last,
                             'E_inference_uJ': E_inf['E_total_uJ'],
                             'n_params': E_inf['n_params'], 'time_sec': res.get('time_sec', 0)})
                print(f"      F={res['F_best']:.4f}, E_inf={E_inf['E_total_uJ']:.4f} uJ")
                del res; gpu_cleanup()
...
print("Defined: optimize_with_unified_output, run_p_sweep_scnn")
...
print(f"Models per (p, method): 6  (CNN-Simple2D, CNN-Up2D-Paper, SCNN-Norse-Simple2D,")
print(f"                            SCNN-Norse-Up2D, SCNN-Crossbar-8b, SCNN-Crossbar-4b)")
print(f"Total runs: {len(P_LIST)} p-values * 2 methods * 6 models = {len(P_LIST)*2*6}")
...
# =============================================================================
COLORS = {'CNN-Simple2D':'#1f77b4', 'CNN-Up2D-Paper':'#17becf',
          'SCNN-Norse-Simple2D':'#2ca02c', 'SCNN-Norse-Up2D':'#bcbd22',
          'SCNN-Crossbar-Simple2D-8b':'#ff7f0e', 'SCNN-Crossbar-Simple2D-4b':'#d62728'}
MARKERS = {'CNN-Simple2D':'o', 'CNN-Up2D-Paper':'^',
           'SCNN-Norse-Simple2D':'s', 'SCNN-Norse-Up2D':'D',
           'SCNN-Crossbar-Simple2D-8b':'P', 'SCNN-Crossbar-Simple2D-4b':'X'}
...
for ax_idx, method in enumerate(['M1', 'M2']):
    ...
    ax.set_title(f'SCNN p-sweep — {method} (N={N_FIXED}, T={T_FIXED})', fontsize=13)
...
plt.tight_layout()
plt.savefig('p_sweep_SCNN_fidelity_vs_p.png', dpi=150, bbox_inches='tight')
plt.savefig('p_sweep_SCNN_fidelity_vs_p.pdf', bbox_inches='tight')
print("Saved: p_sweep_SCNN_fidelity_vs_p.png/pdf")
