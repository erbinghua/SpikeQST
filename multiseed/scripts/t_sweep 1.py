import sys
#!/usr/bin/env python3
"""AUTO-GENERATED from T_sweep_GHZ.ipynb.
...
# Fixed SNN hyperparams (same for all T)
V_TH = 0.9
ENC_GAMMA = 1.5
TAU_MEM_INV = 100.0
TAU_SYN_INV = 200.0
...
# CHECKPOINT_DIR = Path("checkpoints_T_sweep")
# CHECKPOINT_DIR.mkdir(exist_ok=True)
# 
# 
def make_model_fn(config, T, N):
    """Build a model factory for the given config, T, and N.
    All hyperparams except T are fixed at sweep defaults."""
    name = config['name']
    bits = config['bits']
    ...
    # Crossbar: adapt v_th/gamma per bits (needed for spiking with quantized weights)
    if bits <= 4:
...
def run_T_sweep(T_list=T_LIST, N_list=N_LIST, methods=METHODS,
                ...
                checkpoint_every=10):
    """
    Sweep T for all (N, method, model) combinations.

    Returns pd.DataFrame with columns:
        Model, N, method, T, F_best, F_last,
    ...
    if _ARGS.quick:
        ...
        print(f"[quick] T={T_list} N={N_list}", flush=True)
            ...
            for T_val in T_list:
                for config in model_configs:
                    ...
                    if (SEED, name, N, method, float(T_val)) in _done_keys:
                        print(f"  [skip] seed={SEED} {name} N={N} {method} T={T_val}", flush=True)
                        continue

                    print(f"\n[{run_idx}/{total}] {method} | N={N} | T={T_val} | {name}")
                    ...
                    try:
                        ...
                        ckpt_name = f'seed{SEED}_{name}_N{N}_{method}_T{T_val}.pt'
                        torch.save({
                            'state_dict': model.state_dict(),
                            'model_name': name, 'N': N, 'method': method, 'T': T_val,
...
# df_sweep = pd.read_csv(CHECKPOINT_DIR / 'T_sweep_final.csv')
# display(df_sweep)
...
for method in METHODS:
    ...
    pivot = df_m.pivot_table(
        index=['Model', 'N'], columns='T',
        values='F_best', aggfunc='first')

print("\n" + "=" * 80)
print("INFERENCE ENERGY (uJ) vs T")
print("=" * 80)
...
# Verify parameter count is constant across T
print("\n--- Verification: n_params constant across T ---")
for method in METHODS:
    df_m = df_sweep[df_sweep['method'] == method]
    check = df_m.groupby(['Model', 'N'])['n_params'].nunique()
    if (check == 1).all():
        print(f"  {method}: OK")
    else:
        print(f"  {method}: WARNING - params vary!")
        print(check[check > 1])


# === cell #20 ===
# =====================================================================
# PLOT 1: Infidelity (1-F) vs T
# =====================================================================
...
fig.suptitle('Infidelity vs SNN Timesteps T', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot1_infidelity_vs_T.pdf', bbox_inches='tight', dpi=150)


# === cell #21 ===
# =====================================================================
# PLOT 3: Fidelity vs Energy Pareto
# =====================================================================
...
T_log = np.log2(np.array(T_LIST, dtype=float))
norm = mcolors.Normalize(vmin=T_log.min(), vmax=T_log.max())
cmap = plt.cm.viridis
...
sm.set_array([])
cbar = fig.colorbar(sm, ax=axes, label='log2(T)', shrink=0.8)
cbar.set_ticks(T_log)
cbar.set_ticklabels([str(t) for t in T_LIST])
...
fig.suptitle('Fidelity vs Inference Energy (colored by T)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot3_pareto_F_vs_E.pdf', bbox_inches='tight', dpi=150)
...
for method in METHODS:
    ...
    for cfg in MODEL_CONFIGS:
        ...
        for N in N_LIST:
            df_mn = df_model[df_model['N'] == N].sort_values('T')
            if len(df_mn) == 0: continue

            best_row = df_mn.loc[df_mn['F_best'].idxmax()]
            T_best = int(best_row['T'])
            F_at_best = best_row['F_best']
            E_at_best = best_row['E_inference_uJ']

            # Diminishing returns: F at T=64 vs T=128
            F_64 = df_mn[df_mn['T'] == 64]['F_best'].values
            F_128 = df_mn[df_mn['T'] == 128]['F_best'].values
            if len(F_64) > 0 and len(F_128) > 0:
                gain = F_128[0] - F_64[0]
                remaining = 1 - F_64[0]
                pct = gain / remaining * 100 if remaining > 1e-6 else 0
                dimret = f"F(128)-F(64) = {gain:+.4f} ({pct:.1f}% of remaining gap)"
            else:
                dimret = "N/A"

            print(f"  {name}, N={N}: best T={T_best} "
                  f"(F={F_at_best:.4f}, E={E_at_best:.4f}uJ) | {dimret}")

# Optimal T per model (best F/E ratio with F >= 0.95)
print("\n" + "=" * 80)
print("OPTIMAL T (best fidelity with F >= 0.95)")
print("=" * 80)
for method in METHODS:
    df_m = df_sweep[df_sweep['method'] == method]
    print(f"\n--- {method} ---")
    for cfg in MODEL_CONFIGS:
        name = cfg['name']
        for N in N_LIST:
            df_mn = df_m[(df_m['Model'] == name) & (df_m['N'] == N)]
            good = df_mn[df_mn['F_best'] >= 0.95].sort_values('T')
            if len(good) > 0:
                first = good.iloc[0]
                print(f"  {name}, N={N}: T>={int(first['T'])} needed for F>=0.95 "
                      f"(F={first['F_best']:.4f}, E={first['E_inference_uJ']:.4f}uJ)")
            else:
                print(f"  {name}, N={N}: F>=0.95 NOT achieved at any T")
...
def recompute_T_sweep_energy(T_list=T_LIST, N_list=N_LIST, methods=METHODS,
                              model_configs=MODEL_CONFIGS):
    """Reload models from checkpoints and recompute energy for all T sweep points."""
    rows = []

    print("="*80)
    print("ENERGY RECOMPUTE FROM T-SWEEP CHECKPOINTS")
    print("="*80)
    ...
    for method in methods:
        ...
        for N in N_list:
            for T_val in T_list:
                for config in model_configs:
                    name = config['name']
                    ckpt_path = CHECKPOINT_DIR / f'{name}_N{N}_{method}_T{T_val}.pt'
                    ...
                    row = {
                        'Model': name, 'N': N, 'method': method, 'T': T_val,
                        'F_best': ckpt['F_best'],
                        'F_last': ckpt.get('F_last', ckpt['F_best']),
                        'E_inference_uJ': E_inf['E_total_uJ'],
                        'n_params': n_params,
                        'time_sec': ckpt['time_sec'],
                    }
                    rows.append(row)
                    print(f"  {name} N={N} {method} T={T_val}: E={E_inf['E_total_uJ']:.4f}uJ")

                    del model

    df = pd.DataFrame(rows)
    csv_path = CHECKPOINT_DIR / 'T_sweep_recomputed.csv'
    df.to_csv(csv_path, index=False)
    print(f"\nRecomputed {len(df)} rows -> {csv_path}")
    return df

# Usage:
# df = recompute_T_sweep_energy()
# Then use existing plot functions with df
