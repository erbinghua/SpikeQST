import sys
#!/usr/bin/env python3
"""AUTO-GENERATED from capacity_sweep_GHZ.ipynb.
...
def estimate_cnn_energy(model, input_shape, steps=1):
    ...
    macs_dict_mem = count_macs_per_inference(model, T=1)
    n_memory_accesses = sum(macs_dict_mem.values())
...
class CrossbarLIFConv2d(NorseLIFBase):
    ...
    def __init__(self, c_in, c_out, k=3, s=1, p=1, bias=False,
                 ...
                 **kwargs):
        # All crossbar params consumed here; only Norse params flow via **kwargs
        super().__init__(output_features=c_out, **kwargs)
...
def optimize_single_state_crossbar(model_class, N=3, method='M1', M=12, steps=200, lr=1e-3,
    ...
    if warm_spiking:
        warm_init_spiking(G, w_scale=5.0, bias=0.1)
    ...
    scaler = torch.amp.GradScaler('cuda', enabled=use_amp_actual)
    ...
    with torch.no_grad():
        ...
        try:
            ...
            energy_final = {'E_total_J': 0, 'E_total_mJ': 0}
    ...
    for method in methods:
        ...
        for N in N_list:
            ...
            for model_name, model_fn, is_spiking in model_configs:
                ...
                results[f'{method}_N{N}_{model_name}'] = {
                    'F': res['F_best'],
                    'model': res['model'],
                ...
                # Print summary
                print(f"    F={res['F_best']:.4f}, Time={elapsed:.1f}s")

    return results


# =============================================================================
# Compare Crossbar vs Standard at different bit-widths
# =============================================================================

def compare_crossbar_vs_standard(N=3, method='M1', M=12, steps=400, lr=1e-3):
    """Compare standard SCNN with crossbar versions at 4, 8-bit."""

    print(f"\n{'='*70}")
    print(f"📊 CROSSBAR vs STANDARD COMPARISON: N={N}, {method}")
    print(f"{'='*70}")
    ...
    for bits in bit_configs:
        ...
        print(f"  {hw_type}: F={res['F_best']:.4f}")

    return results

print("✓ optimize_single_state_crossbar(), run_crossbar_benchmark(), compare_crossbar_vs_standard() defined")
...
def optimize_single_state(model_class, N=3, method='M1', M=12, steps=200, lr=1e-3,
    ...
    return {
        'model': G, 'F_best': F_final, 'f_hist': np.array(f_hist), 'l_hist': np.array(l_hist),
...
# === cell #14 ===
# =============================================================================
# FLEXIBLE MODEL CLASSES (variable width and depth)
# =============================================================================

class CNNGen_Simple2D_Flex(nn.Module):
    """CNN generator with configurable width (channels) and depth (conv layers)."""

    def __init__(self, cond_dim, d, proj_hw=(10, 10), ch=(32, 64),
                 n_conv_layers=2, dropout=0.0):
        super().__init__()
        H0, W0 = proj_hw
        C0, C1 = ch
        self.d, self.C0, self.HW = d, C0, (H0, W0)
        ...
        # Build conv stack: first layer C0→C1, rest C1→C1
        convs = []
        norms = []
        in_c = C0
        for i in range(n_conv_layers):
            out_c = C1
            convs.append(nn.Conv2d(in_c, out_c, 3, padding=1, bias=False))
            norms.append(nn.InstanceNorm2d(out_c, affine=True))
            in_c = out_c
...
class SCNNGen_LIF_Simple2D_Flex(nn.Module):
    """SCNN-Norse generator with configurable width and depth."""

    def __init__(self, cond_dim, d, proj_hw=(10, 10), ch=(32, 64),
                 n_conv_layers=2, T=8, tau_mem_inv=100.0, tau_syn_inv=200.0,
                 v_th=1.0, enc_mode='poisson', enc_gamma=1.5):
        super().__init__()
        H0, W0 = proj_hw
        C0, C1 = ch
        ...
        for i in range(n_conv_layers):
            out_c = C1
            convs.append(NorseLIFConv2d(in_c, out_c, k=3, p=1, **kw_spk))
            in_c = out_c
        self.convs = nn.ModuleList(convs)
...
class SCNNGen_Crossbar_Simple2D_Flex(nn.Module):
    """SCNN-Crossbar generator with configurable width and depth."""

    def __init__(self, cond_dim, d, proj_hw=(10, 10), ch=(32, 64),
                 n_conv_layers=2, T=8, tau_mem_inv=100.0, tau_syn_inv=200.0,
                 v_th=1.0, weight_bits=8, adc_bits=8, dac_bits=8,
                 noise_std=0.01, device_variation=0.02,
                 enc_mode='poisson', enc_gamma=1.5):
        super().__init__()
        H0, W0 = proj_hw
        ...
        for i in range(n_conv_layers):
            out_c = C1
            convs.append(CrossbarLIFConv2d(in_c, out_c, k=3, p=1, **kw_spk))
            in_c = out_c
        self.convs = nn.ModuleList(convs)
    ...
    def get_model_energy(self, batch_size=1, input_sparsity=0.1):
        layers = [('proj', self.proj)]
        for i, conv in enumerate(self.convs):
            layers.append((f'conv{i}', conv))
        layers.append(('ro', self.ro))
        total_energy = 0
        layer_breakdown = {}
        for name, layer in layers:
            if hasattr(layer, 'get_layer_energy'):
...
print("✓ Flexible model classes defined (CNNGen_Simple2D_Flex, SCNNGen_LIF_Simple2D_Flex, SCNNGen_Crossbar_Simple2D_Flex)")
...
# Width sweep: vary channels at fixed depth=2
WIDTH_CONFIGS = [
    {'label': 'XS',  'ch': (4, 8),     'depth': 2, 'axis': 'width'},
    {'label': 'S',   'ch': (8, 16),    'depth': 2, 'axis': 'width'},
    {'label': 'M',   'ch': (16, 32),   'depth': 2, 'axis': 'width'},
    {'label': 'L',   'ch': (32, 64),   'depth': 2, 'axis': 'width'},
    {'label': 'XL',  'ch': (64, 128),  'depth': 2, 'axis': 'width'},
    {'label': 'XXL', 'ch': (128, 256), 'depth': 2, 'axis': 'width'},
]

# Depth sweep: vary layers at fixed width ch=(32,64)
DEPTH_CONFIGS = [
    {'label': 'D1', 'ch': (32, 64), 'depth': 1, 'axis': 'depth'},
    # D2 = same as 'L' in width sweep, skip to avoid duplicate
    {'label': 'D3', 'ch': (32, 64), 'depth': 3, 'axis': 'depth'},
    {'label': 'D4', 'ch': (32, 64), 'depth': 4, 'axis': 'depth'},
]

ALL_CONFIGS = WIDTH_CONFIGS + DEPTH_CONFIGS

# Model configurations
# NOTE: v_th values are calibrated for Norse LIF dynamics.
# Norse LIF uses two-stage integration (synapse + membrane), so voltage
# accumulates much slower than simple first-order LIF.  With tau_mem_inv=100,
# tau_syn_inv=200, T=8: v(8) ≈ 1.8 × I_input.  Projection layer (fan_in=M)
# with w_scale=5.0 produces currents ~0.17, so v_th must be < 0.17*1.8 ≈ 0.31.
# If v_th is too high, ALL neurons are silent → DensityMap gets zero input →
# gradient of A@A†/tr is zero at A=0 → complete gradient blockage → no learning.
MODEL_CONFIGS = [
    {
        'name': 'CNN',
        'hw': 'GPU',
        'make_model': lambda cond_dim, d, ch, depth: CNNGen_Simple2D_Flex(
            cond_dim=cond_dim, d=d, proj_hw=PROJ_HW, ch=ch, n_conv_layers=depth),
        'warm_spiking': False,
        'is_crossbar': False,
    },
    {
        'name': 'SCNN-Norse',
        'hw': 'Loihi',
        'make_model': lambda cond_dim, d, ch, depth: SCNNGen_LIF_Simple2D_Flex(
            cond_dim=cond_dim, d=d, proj_hw=PROJ_HW, ch=ch, n_conv_layers=depth,
            T=T_FIXED, v_th=0.15, enc_gamma=1.5),
        'warm_spiking': True,
        'is_crossbar': False,
    },
    {
        'name': 'SCNN-Crossbar-8b',
        'hw': 'Crossbar-8b',
        'make_model': lambda cond_dim, d, ch, depth: SCNNGen_Crossbar_Simple2D_Flex(
            cond_dim=cond_dim, d=d, proj_hw=PROJ_HW, ch=ch, n_conv_layers=depth,
            T=T_FIXED, v_th=0.1, enc_gamma=2.25,
            weight_bits=8, adc_bits=8, dac_bits=8,
            noise_std=0.01, device_variation=0.02),
        'warm_spiking': True,
        'is_crossbar': True,
    ...
    {
        'name': 'SCNN-Crossbar-4b',
        'hw': 'Crossbar-4b',
        'make_model': lambda cond_dim, d, ch, depth: SCNNGen_Crossbar_Simple2D_Flex(
            cond_dim=cond_dim, d=d, proj_hw=PROJ_HW, ch=ch, n_conv_layers=depth,
            T=T_FIXED, v_th=0.08, enc_gamma=3.0,
            weight_bits=4, adc_bits=4, dac_bits=4,
            noise_std=0.01, device_variation=0.02),
        'warm_spiking': True,
        'is_crossbar': True,
        'bits': 4,
...
total_runs = len(ALL_CONFIGS) * len(MODEL_CONFIGS) * len(METHODS)
print(f"Capacity Sweep Configuration:")
print(f"  N={N}, d={D}, T={T_FIXED}")
print(f"  Width configs: {len(WIDTH_CONFIGS)}")
print(f"  Depth configs: {len(DEPTH_CONFIGS)} (+1 shared with width)")
...
# === cell #16 ===
# =============================================================================
# CAPACITY SWEEP FUNCTION
# =============================================================================

def run_capacity_sweep():
    # === MULTISEED PATCHED ===
    global METHODS, ALL_CONFIGS, MODEL_CONFIGS
    if _ARGS.quick:
        METHODS[:] = METHODS[:1]
    ...
    if CSV_PATH.exists():
        ...
        _done_keys = {(int(r['seed']), r['method'], r['axis'], r['label'],
                       r['model']) for r in results if r.get('seed') == SEED}
        print(f"  [resume] {len(_done_keys)} rows for seed={SEED}", flush=True)
    else:
        results = []
        _done_keys = set()
    """Run the full capacity sweep: width + depth × models × methods."""
    import gc
    from pathlib import Path

    ckpt_dir = CHECKPOINT_DIR  # (multiseed)
    d = D
    results = []
    run_idx = 0
    total = len(ALL_CONFIGS) * len(MODEL_CONFIGS) * len(METHODS)
    csv_path = CSV_PATH
    for method in METHODS:
        M = M_M1 if method == 'M1' else M_M2
        ...
        for cfg in ALL_CONFIGS:
            ...
            for mcfg in MODEL_CONFIGS:
                ...
                if (SEED, method, axis, label, model_name) in _done_keys:
                    print(f"  [skip] seed={SEED} {method} {axis}:{label} {model_name}", flush=True)
                    continue

                print(f"\n[{run_idx}/{total}] {method} | {axis}:{label} ch={ch} depth={depth} | {model_name}")

                # Build model factory (takes cond_dim, d → returns model)
                def make_model_fn(_mcfg=mcfg, _ch=ch, _depth=depth):
                    def fn(cond_dim, d_):
                        return _mcfg['make_model'](cond_dim, d_, _ch, _depth)
                    return fn

                model_fn = make_model_fn()

                # Count params by building a dummy model
                try:
                    cond_dim_test = M if method == 'M1' else M * d
                    dummy = model_fn(cond_dim_test, d)
                    ...
                    results.append({
                        'seed': SEED,
                        'method': method, 'axis': axis, 'label': label,
                        'C0': ch[0], 'C1': ch[1], 'depth': depth,
                        'model': model_name, 'hw': mcfg['hw'],
                        'n_params': 0, 'F_best': 0.0,
                        'E_inference_uJ': 0.0, 'time_sec': 0.0,
                    })
                    continue
                ...
                # Train using existing optimize functions
                t0 = time.time()
                try:
                    # AMP is safe for spiking models: DensityMap and fidelity
                    # already disable autocast internally for complex ops
                    use_amp = True

                    if mcfg.get('is_crossbar', False):
                        bits = mcfg.get('bits', 8)
                        res = optimize_single_state_crossbar(
                            model_class=model_fn,
                            N=N, method=method, M=M, steps=(3 if _ARGS.quick else STEPS), lr=LR,
                            warm_spiking=True,
                            weight_bits=bits, adc_bits=bits, dac_bits=bits,
                            device_variation=0.02, read_noise=0.01,
                            use_amp=use_amp
                        ...
                        res = optimize_single_state(
                            model_class=model_fn,
                            N=N, method=method, M=M, steps=(3 if _ARGS.quick else STEPS), lr=LR,
                            warm_spiking=mcfg.get('warm_spiking', False),
                            use_amp=use_amp
                        )
                    elapsed = time.time() - t0
                    F_best = res['F_best']
                ...
                if model_trained is not None:
                    try:
                        if model_name == 'CNN':
                            E_inf = estimate_gpu_inference_energy(model_trained)
                        elif model_name == 'SCNN-Norse':
                            E_inf = estimate_loihi_inference_energy(model_trained, T=T_FIXED, sparsity=SPARSITY)
                        else:
                            bits = mcfg.get('bits', 8)
                            E_inf = estimate_crossbar_inference_energy(model_trained, T=T_FIXED, sparsity=SPARSITY, bits=bits)
                        E_uJ = E_inf['E_total_uJ']
                    except Exception as e:
                        print(f"  WARNING energy estimation failed: {e}")

                print(f"  ✓ F={F_best:.4f}, params={n_params:,}, E={E_uJ:.4f}µJ, time={elapsed:.1f}s")
                ...
                results.append({
                    ...
                    'C0': ch[0], 'C1': ch[1], 'depth': depth,
                    'model': model_name, 'hw': mcfg['hw'],
                    'n_params': n_params, 'F_best': F_best,
                    'E_inference_uJ': E_uJ, 'time_sec': elapsed,
                })

                # Save model checkpoint
                if model_trained is not None:
                    ckpt_name = f'seed{SEED}_{model_name}_{label}_{method}.pt'
                    torch.save({
                        'state_dict': model_trained.state_dict(),
                        'model_name': model_name, 'label': label,
                        'ch': ch, 'depth': depth, 'axis': axis,
                        'method': method, 'N': N,
                        'F_best': F_best, 'time_sec': elapsed,
                        'n_params': n_params,
                        'mcfg_name': mcfg['name'],
                        'bits': mcfg.get('bits', None),
                    }, ckpt_dir / ckpt_name)
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                gc.collect()
                pd.DataFrame(results).to_csv(csv_path, index=False)  # multiseed: save every iter

    df = pd.DataFrame(results)
    df.to_csv(csv_path, index=False)
    print(f"\n{'='*60}")
    print(f"✓ Capacity sweep complete: {len(df)} runs saved to {csv_path}")
    return df

print("✓ run_capacity_sweep() defined")
...
# df_sweep = pd.read_csv(f'capacity_sweep_N{N}_T{T_FIXED}.csv')

print("=" * 80)
print("WIDTH SWEEP RESULTS")
print("=" * 80)

df_width = df_sweep[df_sweep['axis'] == 'width'].copy()
for method in METHODS:
    print(f"\n--- {method} ---")
    pivot = df_width[df_width['method'] == method].pivot_table(
        index='label', columns='model', values='F_best', aggfunc='first')
    # Reorder
    ...
    # Also show params
    params_pivot = df_width[df_width['method'] == method].pivot_table(
        index='label', columns='model', values='n_params', aggfunc='first')
    params_pivot = params_pivot.reindex([l for l in label_order if l in params_pivot.index])
    print(f"\nParameter counts:")
    print(params_pivot.to_string(float_format=lambda x: f'{int(x):,}'))

print(f"\n{'=' * 80}")
print("DEPTH SWEEP RESULTS")
print("=" * 80)

# Include D2 = L from width sweep
df_depth_extra = df_sweep[df_sweep['axis'] == 'depth'].copy()
df_L = df_width[df_width['label'] == 'L'].copy()
df_L['axis'] = 'depth'
df_L['label'] = 'D2'
df_depth = pd.concat([df_depth_extra, df_L], ignore_index=True)

for method in METHODS:
    print(f"\n--- {method} ---")
    pivot = df_depth[df_depth['method'] == method].pivot_table(
        index='label', columns='model', values='F_best', aggfunc='first')
    depth_order = ['D1', 'D2', 'D3', 'D4']
...
# === cell #19 ===
# =============================================================================
# PLOT 2: INFIDELITY (1-F) vs DEPTH (depth sweep)
# =============================================================================

# Combine depth sweep with D2=L from width sweep
df_depth_extra = df_sweep[df_sweep['axis'] == 'depth'].copy()
df_L = df_sweep[(df_sweep['axis'] == 'width') & (df_sweep['label'] == 'L')].copy()
df_L['axis'] = 'depth'
df_L['label'] = 'D2'
df_d = pd.concat([df_depth_extra, df_L], ignore_index=True)

fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=True)

for ax, method in zip(axes, METHODS):
    df_m = df_d[df_d['method'] == method]
    for model_name in ['CNN', 'SCNN-Norse', 'SCNN-Crossbar-8b', 'SCNN-Crossbar-4b']:
        df_mod = df_m[df_m['model'] == model_name].sort_values('depth')
        infidelity = 1.0 - df_mod['F_best'].values
        infidelity = np.maximum(infidelity, 1e-6)
        ax.plot(df_mod['depth'].values, infidelity,
...
axes[0].set_ylabel('Infidelity (1 − F)', fontsize=12)
fig.suptitle(f'Depth Sweep: Infidelity vs Conv Depth (N={N}, T={T_FIXED}, ch=(32,64))', fontsize=14)
plt.tight_layout()
plt.savefig('plot2_infidelity_vs_depth.pdf', dpi=150, bbox_inches='tight')
print("✓ Saved plot2_infidelity_vs_depth.pdf")


# === cell #20 ===
# =============================================================================
# PLOT 5: WIDTH vs DEPTH — Parameter-matched comparison
# =============================================================================

# For each depth config, find the width config with closest param count
# Then compare fidelity: is it better to add depth or width for the same budget?

df_d = pd.concat([
    df_sweep[df_sweep['axis'] == 'depth'],
    df_sweep[(df_sweep['axis'] == 'width') & (df_sweep['label'] == 'L')].assign(axis='depth', label='D2')
], ignore_index=True)
...
for ax, method in zip(axes, METHODS):
    for i, model_name in enumerate(['CNN', 'SCNN-Norse', 'SCNN-Crossbar-8b', 'SCNN-Crossbar-4b']):
        fids = []
        for dl in depth_labels:
            row = df_d[(df_d['method'] == method) & (df_d['model'] == model_name) & (df_d['label'] == dl)]
            fids.append(row['F_best'].values[0] if len(row) > 0 else 0)
        offset = (i - 1.5) * bar_width
        bars = ax.bar([xi + offset for xi in x], fids, bar_width,
    ...
    # Set y-axis to focus on high fidelity region
    all_fids = df_d[df_d['method'] == method]['F_best'].values
    if len(all_fids) > 0 and min(all_fids) > 0.8:
        ax.set_ylim(min(all_fids) - 0.02, 1.005)

fig.suptitle(f'Depth Comparison at ch=(32,64) (N={N}, T={T_FIXED})', fontsize=14)
plt.tight_layout()
plt.savefig('plot5_width_vs_depth.pdf', dpi=150, bbox_inches='tight')
print("✓ Saved plot5_width_vs_depth.pdf")
...
print("=" * 80)
print("KEY FINDINGS — CAPACITY SWEEP")
print("=" * 80)

df_w = df_sweep[df_sweep['axis'] == 'width']
...
for method in METHODS:
    ...
    df_m = df_w[df_w['method'] == method]

    for model_name in ['CNN', 'SCNN-Norse', 'SCNN-Crossbar-8b', 'SCNN-Crossbar-4b']:
        df_mod = df_m[df_m['model'] == model_name].sort_values('n_params')
        if len(df_mod) == 0:
            continue

        best_row = df_mod.loc[df_mod['F_best'].idxmax()]
        best_F = best_row['F_best']

        # Find smallest config achieving >99% of best fidelity
        threshold = best_F * 0.99
        efficient = df_mod[df_mod['F_best'] >= threshold]
        if len(efficient) > 0:
            min_row = efficient.loc[efficient['n_params'].idxmin()]
            print(f"  {model_name:25s}: best F={best_F:.4f} @ {best_row['label']} ({int(best_row['n_params']):,} params)")
            print(f"    → 99% threshold ({threshold:.4f}) met at {min_row['label']} ({int(min_row['n_params']):,} params, F={min_row['F_best']:.4f})")
        else:
            print(f"  {model_name:25s}: best F={best_F:.4f} @ {best_row['label']} ({int(best_row['n_params']):,} params)")

# Width vs Depth comparison
print(f"\n{'─' * 60}")
print("  WIDTH vs DEPTH COMPARISON (ch=(32,64))")
print(f"{'─' * 60}")
...
for method in METHODS:
    ...
    for model_name in ['CNN', 'SCNN-Norse', 'SCNN-Crossbar-8b', 'SCNN-Crossbar-4b']:
        ...
        if len(df_mod) == 0:
            continue
        fids = df_mod.set_index('label')['F_best']
        depth_str = ', '.join([f"D{d}={fids.get(f'D{d}', 0):.4f}" for d in [1,2,3,4] if f'D{d}' in fids.index])
        best_depth = df_mod.loc[df_mod['F_best'].idxmax()]
        print(f"    {model_name:25s}: {depth_str}")
        print(f"      → best depth={int(best_depth['depth'])} (F={best_depth['F_best']:.4f})")
...
def recompute_capacity_sweep_energy():
    """Reload models from checkpoints and recompute energy estimates."""
    from pathlib import Path

    ckpt_dir = Path("checkpoints_capacity_sweep")
    if not ckpt_dir.exists():
        print("ERROR: No checkpoints found. Run the full sweep first.")
        return None
    ...
    print("="*80)
    print("ENERGY RECOMPUTE FROM CAPACITY SWEEP CHECKPOINTS")
    print("="*80)
    ...
    for method in METHODS:
        ...
        for cfg in ALL_CONFIGS:
            ...
            for mcfg in MODEL_CONFIGS:
                model_name = mcfg['name']
                ckpt_path = ckpt_dir / f'{model_name}_{label}_{method}.pt'
                ...
                try:
                    model = model_fn(cond_dim, d).to(DEVICE)
                ...
                try:
                    if model_name == 'CNN':
                        ...
                        E_inf = estimate_loihi_inference_energy(model, T=T_FIXED, sparsity=SPARSITY)
                    else:
                        bits = mcfg.get('bits', 8)
                        E_inf = estimate_crossbar_inference_energy(model, T=T_FIXED, sparsity=SPARSITY, bits=bits)
                    E_uJ = E_inf['E_total_uJ']
                except Exception as e:
                    print(f"  WARNING energy failed for {ckpt_path.name}: {e}")

                n_params = ckpt.get('n_params', sum(p.numel() for p in model.parameters()))
                print(f"  {model_name} {label} {method}: F={ckpt['F_best']:.4f}, E={E_uJ:.4f}uJ")
                ...
                results.append({
                    ...
                    'C0': ch[0], 'C1': ch[1], 'depth': depth,
                    'model': model_name, 'hw': mcfg['hw'],
                    'n_params': n_params, 'F_best': ckpt['F_best'],
                    'E_inference_uJ': E_uJ, 'time_sec': ckpt['time_sec'],
                })
                del model
    ...
    csv_path = CSV_PATH
    df.to_csv(csv_path, index=False)
    print(f"\nRecomputed {len(df)} rows -> {csv_path}")
    return df

# Usage:
# df = recompute_capacity_sweep_energy()
# Then use existing plot functions
