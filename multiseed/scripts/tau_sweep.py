import sys
#!/usr/bin/env python3
"""AUTO-GENERATED from tau_sweep_GHZ.ipynb.
...
# === cell #15 ===
# =====================================================================
# TAU (NEURON TIME-CONSTANT) SWEEP CONFIGURATION
# =====================================================================
# Coupled 1:2 sweep: tau_syn_inv = 2 * tau_mem_inv (physically motivated:
# synaptic current decays twice as fast as membrane voltage).

TAU_MEM_INV_LIST = [25.0, 50.0, 100.0, 200.0, 400.0]
N_LIST = [3, 5, 8]
METHODS = ['M1', 'M2']
M_M1 = 12
M_M2 = 2
...
# Fixed SNN hyperparams (same for all tau)
T_FIXED = 16
V_TH = 0.9
ENC_GAMMA = 1.5
...
total = len(TAU_MEM_INV_LIST) * len(N_LIST) * len(METHODS) * len(MODEL_CONFIGS)
print(f"Tau-Sweep Configuration:")
print(f"  tau_mem_inv values: {TAU_MEM_INV_LIST}")
print(f"  tau_syn_inv values: {[2.0*t for t in TAU_MEM_INV_LIST]} (coupled 1:2)")
print(f"  N values:           {N_LIST}")
print(f"  Methods:            {METHODS}")
print(f"  Models:             {[c['name'] for c in MODEL_CONFIGS]}")
print(f"  Fixed T:            {T_FIXED}")
print(f"  Fixed V_th:         {V_TH}")
...
# CHECKPOINT_DIR = Path("checkpoints_tau_sweep")
...
def make_model_fn(config, tau_mem_inv_val, N):
    """Build a model factory with coupled tau_syn_inv = 2 * tau_mem_inv_val."""
    name = config['name']
    bits = config['bits']
    tau_syn_inv_val = 2.0 * tau_mem_inv_val
    ...
    if name == 'Norse-Simple2D':
        def fn(cond_dim, d):
            return SCNNGen_LIF_Simple2D(
                ...
                tau_mem_inv=tau_mem_inv_val, tau_syn_inv=tau_syn_inv_val)
        return fn
...
def run_tau_sweep(tau_list=None, N_list=None, methods=None,
                  model_configs=None, use_amp=True, checkpoint_every=10):
    tau_list = tau_list or TAU_MEM_INV_LIST
    ...
    total = len(tau_list) * len(N_list) * len(methods) * len(model_configs)
    run_idx = 0
        ...
        for N in N_list:
            ...
            for tau_val in tau_list:
                for config in model_configs:
                    run_idx += 1
                    name = config['name']
                    if (SEED, name, N, method, float(tau_val)) in _done_keys:
                        print(f"  [skip] seed={SEED} {name} N={N} {method} tau_mem_inv={tau_val}", flush=True)
                        continue
                    print(f"\n[{run_idx}/{total}] {method} | N={N} | tau_mem_inv={tau_val} | {name}")

                    model_fn = make_model_fn(config, tau_val, N)
                    ...
                    try:
                        ...
                        ckpt_name = f'seed{SEED}_{name}_N{N}_{method}_tau{int(tau_val)}.pt'
                        torch.save({
                            'state_dict': model.state_dict(),
                            'model_name': name, 'N': N, 'method': method,
                            'T': T_FIXED, 'tau_mem_inv': tau_val,
                            'tau_syn_inv': 2.0 * tau_val,
                        ...
                        rows.append({
                            'seed': SEED,
                            'Model': name, 'N': N, 'method': method,
                            'tau_mem_inv': tau_val, 'tau_syn_inv': 2.0 * tau_val,
                            'T': T_FIXED,
...
# df_sweep = pd.read_csv(CHECKPOINT_DIR / 'tau_sweep_final.csv')
# display(df_sweep)
...
for method in METHODS:
    ...
    pivot = df_m.pivot_table(
        index=['Model', 'N'], columns='tau_mem_inv',
        values='F_best', aggfunc='first')

print("\n" + "=" * 80)
print("INFERENCE ENERGY (uJ) vs tau_mem_inv")
print("=" * 80)
for method in METHODS:
    print(f"\n--- Method: {method} ---")
    df_m = df_sweep[df_sweep['method'] == method]
...
print("\n" + "=" * 80)
print("MEAN SPIKE RATE vs tau_mem_inv")
print("=" * 80)
for method in METHODS:
    print(f"\n--- Method: {method} ---")
    df_m = df_sweep[df_sweep['method'] == method]
...
# === cell #20 ===
# =====================================================================
# PLOT 1: Infidelity (1-F) vs tau_mem_inv
# =====================================================================
...
for method in METHODS:
    ...
    for ax_idx, N in enumerate(N_LIST):
        ...
        for cfg in MODEL_CONFIGS:
            name = cfg['name']
            df_model = df_n[df_n['Model'] == name].sort_values('tau_mem_inv')
            if len(df_model) == 0: continue
            infidelity = np.clip(1.0 - df_model['F_best'].values, 1e-6, None)
            ls = '-' if method == 'M1' else '--'
            ax.plot(df_model['tau_mem_inv'].values, infidelity,
                    marker=MARKERS[name], color=COLORS[name],
                    linestyle=ls, label=f"{name} ({method})",
                    linewidth=1.5, markersize=6)
        ax.set_xscale('log', base=10)
        ax.set_yscale('log')
        ax.set_xlabel('tau_mem_inv (Norse decay rate)')
        ax.set_title(f'N = {N} qubits')
        ax.grid(True, alpha=0.3, which='both')
        ax.set_xticks(TAU_MEM_INV_LIST)
        ax.set_xticklabels([str(v) for v in TAU_MEM_INV_LIST])

axes[0].set_ylabel('Infidelity (1 - F)')
handles, labels = axes[-1].get_legend_handles_labels()
fig.legend(handles, labels, loc='upper center',
           bbox_to_anchor=(0.5, -0.02), ncol=3, fontsize=9)
fig.suptitle('Infidelity vs tau_mem_inv (Norse decay rate)', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot1_tau_infidelity.pdf', bbox_inches='tight', dpi=150)


# === cell #21 ===
# =====================================================================
# PLOT 4: Fidelity vs Energy Pareto (colored by tau_mem_inv)
# =====================================================================
...
sm.set_array([])
cbar = fig.colorbar(sm, ax=axes, label='tau_mem_inv (Norse decay rate)', shrink=0.8)
...
fig.suptitle('Fidelity vs Inference Energy (colored by tau_mem_inv (Norse decay rate))', fontsize=14, y=1.02)
plt.tight_layout()
plt.savefig('plot4_tau_pareto.pdf', bbox_inches='tight', dpi=150)
...
print("=" * 80)
print("KEY FINDINGS: best tau_mem_inv per (model, N)")
print("=" * 80)
...
for method in METHODS:
    ...
    for cfg in MODEL_CONFIGS:
        ...
        for N in N_LIST:
            ...
            print(f"  {name}, N={N}: best tau_mem_inv={best_row['tau_mem_inv']} "
...
def recompute_tau_sweep_energy():
    """Reload models from checkpoints and recompute energy for every sweep point."""
    print("="*80)
    print("ENERGY RECOMPUTE FROM TAU_SWEEP CHECKPOINTS")
    print("="*80)
    rows = []
    for ckpt_path in sorted(CHECKPOINT_DIR.glob('*.pt')):
        ...
        model_fn = make_model_fn(cfg, ckpt.get('tau', None), N)
    ...
    df = pd.DataFrame(rows)
    df.to_csv(CSV_PATH, index=False)
    print(f"\nRecomputed {len(df)} rows -> tau_sweep_recomputed.csv")
    return df

# Usage: df = recompute_tau_sweep_energy()
